...
class ThreatDB:
    ...
    def ip_insert(self):
        ...
        """
        query = """INSERT INTO blocked_ips (bad_ips, reliability, priority, activity, sub_category, country, 
                city, latitude, longitude, source, target, dest_port, last_online, first_seen, used_by, reference
    ...
    def ip_update(self):
        ...
        query = """UPDATE blocked_ips SET reliability = %s, priority = %s, activity = %s, sub_category = %s,
                   ...
                   last_online = %s, first_seen = %s, used_by = %s, reference = %s,
                   updated_at = %s WHERE bad_ips = %s;"""
        return query
    ...
    def ip_remove_duplicates(self):
        ...
        query = """DELETE FROM blocked_ips WHERE id NOT IN
                (SELECT A.* FROM (SELECT MIN(id) FROM  blocked_ips GROUP BY bad_ips)A);"""
